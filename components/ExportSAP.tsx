...
function groupBySupplier(
  ...
  for (const sel of selecciones) {
    const key = sel.proveedorCodigo || sel.proveedorNombre || 'Sin proveedor'
    if (!map.has(key)) {
    ...
    <div className="rounded-xl border border-white/[0.08] overflow-hidden">
      ...
      {sinProveedor && (
        <div className="flex items-start gap-2 px-4 py-2.5 border-t border-white/[0.04] bg-amber-500/[0.03]">
          ...
          <p className="text-xs text-amber-400/50">Sin proveedor asignado. Elígelo en &quot;Configurar pedido&quot; antes de copiar.</p>
        </div>
      )}
    </div>
  )
